...
class GitHubAPI:
    ...
    def __init__(self, token: str):
        ...
        self.end_date = "2023-12-31T23:59:59Z"
    ...
    def get_repos_page(self, page: int = 1, per_page: int = 100) -> List[Dict]:
        ...
        params = {
            'q': f'user:evelynmitchell fork:true created:{self.start_date[:10]}..{self.end_date[:10]}',
            'page': page,
            'per_page': per_page,
            'sort': 'created',
            'order': 'desc'
...
class RepoProcessor:
    ...
    def is_from_target_year(self, created_at: str) -> bool:
        """Check if a repository was created in the target year."""
    ...
    def process_repos_page(self, page: int, batch_size: Optional[int] = None) -> Tuple[List[Dict], bool]:
        ...
        for i, repo in enumerate(repos, 1):
            ...
            if not self.is_from_target_year(created_at):
                continue
